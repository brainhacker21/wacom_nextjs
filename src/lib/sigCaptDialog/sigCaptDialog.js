...
class SigCaptDialog {
  ...
  createWindow(width, height) {
    ...
    if (this.config.attachTo) {
      ...
      this.mSignatureWindow.setAttribute(
        "style",
        "touch-action: none;z-index: 1001;"
      );
      ...
      if (this.config.centered) {
        this.mSignatureWindow.style.top =
          window.innerHeight / 2 - height / 2 + "px";
        this.mSignatureWindow.style.left =
          window.innerWidth / 2 - width / 2 + "px";
      } else {
      ...
      if (this.config.hasTitle) {
        ...
        this.mTitleBar.innerHTML =
          '<div style="padding-left:5px;display: table-cell; vertical-align: middle;height:' +
          titleBarHeight +
          'px;">' +
          this.config.title +
          "</div>";
  ...
  createScreenImage(useColor) {
    ...
    for (let i = 0; i < this.mBtns.length; i++) {
      ...
      let textSize = this.fitTextOnCanvas(
        ctx,
        btn.Text,
        bounds,
        this.config.buttonsFont
      );
    ...
    for (let i = 0; i < this.mBtns.length; ++i) {
      ...
      ctx.fillStyle = useColor
        ? this.config.buttons[i].backgroundColor
        : "#ffffff";
      ...
      let xPos =
        bounds.x + (bounds.width / 2 - ctx.measureText(btn.Text).width / 2);
      ...
      button.innerHTML =
        '<div style="line-height:' + yOffset + 'px">' + btn.Text + "</div>";
      ...
      button.style.border =
        this.config.buttons[i].borderWidth +
        "px solid " +
        this.config.buttons[i].borderColor;
    ...
    if (this.sigObj.isEvaluation()) {
      this.drawEvaluationString(
        getLocateString("evaluation"),
        ctx,
        this.canvas.width,
        this.canvas.height - buttonsTop,
        useColor
      );
    }
    ...
    if (this.reason && this.config.reason.visible) {
      ...
      ctx.font =
        this.config.reason.fontSize + "px " + this.config.reason.fontFace;
      ...
      ctx.fillText(
        this.reason,
        this.config.reason.offsetX,
        actualHeight + this.config.reason.offsetY
      );
    }
    ...
    let actualHeight =
      metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;
    dateOffsetY =
      this.canvas.height - buttonsTop - actualHeight - this.config.date.offsetY;
    if (this.config.date.visible) {
      ctx.fillText(
        dateString,
        this.canvas.width - metrics.width - this.config.date.offsetX,
        dateOffsetY
      );
    }
    //}
    ...
    if (this.signatory) {
      ...
      ctx.font =
        this.config.signatory.fontSize + "px " + this.config.signatory.fontFace;
      let metrics = ctx.measureText(this.signatory);
      //let fontHeight = metrics.fontBoundingBoxAscent + metrics.fontBoundingBoxDescent;
      let actualHeight =
        metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;
      signatoryOffsetY =
        dateOffsetY - actualHeight * 2 - this.config.signatory.offsetY;
      if (this.config.signatory.visible) {
        ctx.fillText(
          this.signatory,
          this.canvas.width - metrics.width - this.config.signatory.offsetX,
          dateOffsetY - actualHeight - this.config.signatory.offsetY
        );
      }
    }
    ...
    if (this.config.signingLine.width > 0 && this.config.signingLine.visible) {
      ...
      ctx.moveTo(
        this.config.signingLine.left,
        signatoryOffsetY -
          this.config.signingLine.width -
          this.config.signingLine.offsetY
      );
      ctx.lineTo(
        this.canvas.width - this.config.signingLine.right,
        signatoryOffsetY -
          this.config.signingLine.width -
          this.config.signingLine.offsetY
      );
  ...
  onDown(ev) {
    ...
    if (ev instanceof PointerEvent) {
      ...
      if (
        typeof ev.pressure !== "undefined" &&
        (ev.pointerType === "pen" || ev.pointerType === "stu")
      ) {
      ...
      if (ev.touches && ev.touches[0]) {
        if (
          ...
          ev.touches[0]["force"] > 0 &&
          ev.touches[0]["force"] < 1
        ) {
    ...
    switch (pointerType) {
      case "mouse":
        if (!this.config.source.mouse) return;
        break;
      case "touch":
        if (!this.config.source.touch) return;
        break;
      case "pen":
        if (!this.config.source.pen) return;
        break;
      case "stu":
        if (!this.config.source.stu) return;
        break;
    }
    ...
    if (!this.willEngine) {
      this.willEngine = new this.sigSDK.WillEngine(
        this.config.strokeSize,
        this.hasPressure
      );
    }
    this.willEngine.addPointerData(
      this.sigSDK.Phase.BEGIN,
      x > 0 ? x : 0,
      y > 0 ? y : 0,
      pressure,
      time
    ...
    if (ev instanceof PointerEvent) {
      ...
      if (this.mDown === "out") {
        ...
        if (ev.changedTouches && ev.changedTouches[0]) {
          ...
          if (
            ev.changedTouches[0]["force"] !== "undefined" &&
            ev.changedTouches[0]["force"] > 0
          ) {
  ...
  getCaptureData() {
    ...
    for (let index = 0; index < this.capturedPoints.length; index++) {
      ...
      var point = {
        ...
        is_down:
          this.capturedPoints[index].type == "down" ||
          this.capturedPoints[index].type == "move",
    ...
    const osInfo =
      this.webBrowserData.os.name + " " + this.webBrowserData.os.version;
    const digitizerInfo =
      this.webBrowserData.browser.name +
      " " +
      this.webBrowserData.browser.version;
    const nicInfo = "";
    ...
    const myPromise = new Promise((resolve, reject) => {
      try {
        const promise = this.sigObj.generateSignature(
          this.signatory,
          this.reason,
          where,
          this.integrityType,
